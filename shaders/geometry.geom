...
void build_rectangle(vec3 color, vec4 pos0, vec4 pos1) {
  ...
  gl_Position = pos0 + vec4(pos0_orthogonal, 0.0, 0.0);
  // gl_Position = pos0 + vec4(pos0_orthogonal, 0.0, 0.0) * width / 2.0;
  // gl_Position = pos0 + vec4(pos0_orthogonal, 0.0, 0.0);
  // gl_Position = vec4(0.0, 0.0, 0.0, 0.1);
  // gl_Position = pos0 + vec4(0.1, 0.1, 0.0, 0.0);

  // gl_Position = pos0;
  EmitVertex();

  gl_Position = pos0 + vec4(pos0_orthogonal_1, 0.0, 0.0);
  // gl_Position = pos0 + vec4(pos0_orthogonal_1, 0.0, 0.0) * width / 2.0;
  // gl_Position = pos0 + vec4(pos0_orthogonal_1, 0.0, 0.0);
  // gl_Position = vec4(0.5, 0.5, 0.0, 0.1);
  // gl_Position = pos0 + vec4(0.1, 0.0, 0.0, 0.0);
  EmitVertex();

  gl_Position = pos1 + vec4(pos1_orthogonal_1, 0.0, 0.0);
  // gl_Position = pos1 + vec4(pos1_orthogonal_1, 0.0, 0.0) * width / 2.0;
  // gl_Position = pos1 + vec4(pos1_orthogonal_1, 0.0, 0.0);
  // gl_Position = vec4(-0.75, -0.75, 0.0, 0.1);
  // gl_Position = pos1 + vec4(0.1, 0.1, 0.0, 0.0);
  // gl_Position = pos1;
  EmitVertex();

  gl_Position = pos1 + vec4(pos1_orthogonal, 0.0, 0.0);
  // gl_Position = pos1 + vec4(pos1_orthogonal, 0.0, 0.0) * width / 2.0;
  // gl_Position = pos1 + vec4(pos1_orthogonal, 0.0, 0.0);
  // gl_Position = vec4(-0.25, -0.25, 0.0, 0.1);
  // gl_Position = pos1 + vec4(0.1, 0.0, 0.0, 0.0);
  EmitVertex();
...
void main() {
  ...
  build_rectangle(color, gl_in[0].gl_Position, gl_in[1].gl_Position);
}
